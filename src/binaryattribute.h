...
class BinaryAttribute {
 public:
  ...
  BinaryAttribute(Type type_in, size_t offset_in);
  ...
  template <typename T>
  void ForEachMut(void* data, size_t stride, size_t count,
                  const std::function<void(T*)>& cb) {
    assert(type != Type::Unknown);
    assert(data);
    uint8_t* ptr = reinterpret_cast<uint8_t*>(data);
    for (size_t i = 0; i < count; i++) {
      cb(reinterpret_cast<T*>(ptr + offset));
      ptr += stride;
    }
  }

  template <typename T>
  void ForEach(const void* data, size_t stride, size_t count,
               const std::function<void(const T*)>& cb) const {
    assert(type != Type::Unknown);
    assert(data);
    const uint8_t* ptr = reinterpret_cast<const uint8_t*>(data);
    for (size_t i = 0; i < count; i++) {
      cb(reinterpret_cast<const T*>(ptr + offset));
      ptr += stride;
    }
  }
