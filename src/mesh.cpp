...
std::shared_ptr<Mesh> Mesh::MakeSphere(const std::shared_ptr<UberMaterial>& mat,
  ...
  std::vector<glm::vec3> positions(verts.size());
  for (size_t i = 0; i < verts.size(); i++) {
    normals[i] = verts[i];
    positions[i] = verts[i] * radius;
  }

  // Build GL buffers.
  auto pos_buf = std::make_shared<BufferObject>(GL_ARRAY_BUFFER, positions);
  auto norm_buf = std::make_shared<BufferObject>(GL_ARRAY_BUFFER, normals);
  auto idx_buf = std::make_shared<BufferObject>(GL_ELEMENT_ARRAY_BUFFER, indices);

  auto vao = std::make_shared<VertexArrayObject>();
  vao->SetAttribBuffer(mat->prog()->GetAttribLoc("position"), pos_buf);
  vao->SetAttribBuffer(mat->prog()->GetAttribLoc("normal"), norm_buf);
  vao->SetElementBuffer(idx_buf);

  return std::make_shared<Mesh>(vao, mat, node);
}

std::shared_ptr<Mesh> Mesh::MakeCylinder(const std::shared_ptr<UberMaterial>& mat,
                                         const std::shared_ptr<Node>& node,
...
std::shared_ptr<Mesh> Mesh::MakeCone(const std::shared_ptr<UberMaterial>& mat,
                                     const std::shared_ptr<Node>& node,
                                     glm::vec3 start, glm::vec3 end, float radius,
                                     int num_circle_subdivs, int num_length_subdivs) {
  glm::vec3 axis = end - start;
...
std::shared_ptr<Mesh> Mesh::MakeBoneOctahedron(const std::shared_ptr<UberMaterial>& mat,
                                               const std::shared_ptr<Node>& node,
                                               glm::vec3 start, glm::vec3 end, float radius) {
  glm::vec3 axis = end - start;
